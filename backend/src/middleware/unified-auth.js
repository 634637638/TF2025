...
const checkSinglePermission = (userPermissions, requiredPermission, _moduleType) => {
  ...
  if (moduleMatch) {
    ...
    const requiredAction = requiredPermission.slice(modulePrefix.length + 1)

    // CRUD 管理员只继承基础 CRUD，审批、到账、上传等业务动作必须独立授权。
    if (['view', 'create', 'edit', 'delete'].includes(requiredAction) && isModuleAdmin(userPermissions, modulePrefix)) {
