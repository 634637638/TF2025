...
class MenuModuleLinker {
  ...
  constructor() {
    this.mappingRules = [
      // 直接名称匹配（使用实际的模块 key）
      { menu_pattern: (name) => name === '考勤管理', module_key: 'attendance_attendanceview' },
      { menu_pattern: (name) => name === '工资管理', module_key: 'salary_salaryview' },
      { menu_pattern: (name) => name === '工资模板', module_key: 'salary_salarytemplatesview' },
      ...
      // 模糊匹配规则
      { menu_pattern: (name) => name.includes('考勤'), module_key: 'attendance_attendanceview' },
      { menu_pattern: (name) => name.includes('工资') && name.includes('模板'), module_key: 'salary_salarytemplatesview' },
      { menu_pattern: (name) => name.includes('工资') && !name.includes('模板'), module_key: 'salary_salaryview' },
      { menu_pattern: (name) => /git/i.test(name) && (name.includes('仓库') || name.includes('管理')), module_key: 'system_gitmanagement' },

      // 通用规则：移除"管理"、"我的"等后缀/前缀
      { menu_pattern: (_name) => true, fallback: true }
    ]
  }
  ...
  findModuleKeyForMenu(menuName) {
    ...
    for (const rule of this.mappingRules) {
      if (!rule.fallback && rule.menu_pattern(menuName)) {
        return rule.module_key
      }
    }
  ...
  needsModuleRelink(menu, expectedModuleId, expectedModuleKey) {
    if (!expectedModuleId || !expectedModuleKey) {
      return false
    }
  ...
  async syncAllMenusToModules() {
    ...
    try {
      ...
      for (const menu of activeMenus) {
        ...
        if (moduleId && this.needsModuleRelink(menu, moduleId, moduleKey)) {
          ...
          results.push({
            menu_id: menu.id,
            menu_name: menu.name,
            action: isUnlinked ? 'linked' : 'corrected',
      ...
      log.debug(`📊 同步完成: ${linkedCount} 个已关联, ${correctedCount} 个已纠正, ${notFoundCount} 个未找到匹配`)

      return {
        success: true,
        message: `同步完成: ${linkedCount} 个关联成功, ${correctedCount} 个纠正成功, ${notFoundCount} 个未找到匹配`,
        linked: linkedCount,
  ...
  async getLinkStats() {
    ...
    try {
      ...
      return {
        success: true,
        data: {
          total_menus: totalMenus[0].count,
          linked_menus: linkedMenus[0].count,
          unlinked_menus: unlinkedMenus[0].count,
          total_modules: totalModules[0].count,
          link_rate: totalMenus[0].count > 0
