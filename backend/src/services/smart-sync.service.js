...
class SmartSyncService {
  ...
  identifyTableType(tableName, columns) {
    ...
    const tableTypes = {
      ...
      sales: {
        keywords: ['sale', 'customer_id', 'phone_id', 'sale_time', 'sale_date'],
  ...
  getFieldAliases(field) {
    const aliasMap = {
      ...
      'sale_time': ['salestime', 'sale_date', 'sold_date', 'sell_date'],
      'inventory_time': ['Inventorytime', 'purchase_date', 'buy_date', 'in_date'],
   ...
   * 特殊字段映射规则
   */
  getSpecialMapping(sourceField, _targetColumns) {
    // 品牌名称 → brand_id
   ...
   * 推断匹配字段
   */
  inferKeyFields(tableType, sourceColumns, _targetColumns) {
        ...
        rules: [
          {
            ...
            protectFields: ['sale_price', 'customer_id', 'sale_operator_id', 'sale_time']
          }
        ]
      }
    }
