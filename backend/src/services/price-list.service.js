...
class PriceListService {
  ...
  /**
   * 批量获取价格趋势。
   * 只读取每个价目表最近两条记录，避免管理端逐行请求历史接口。
   */
  async getPriceHistoryTrends(priceListIds = []) {
    try {
      ...
      const [rows] = await this.db.query(`
        SELECT id, price_list_id, wholesale_price, recorded_at
        FROM price_history
        WHERE price_list_id IN (${placeholders})
        ORDER BY price_list_id ASC, recorded_at DESC, id DESC
      `, ids)

      const historyByPriceListId = new Map()
      for (const row of rows) {
        const priceListId = Number(row.price_list_id)
        const history = historyByPriceListId.get(priceListId) || []
        if (history.length < 2) {
          history.push(row)
          historyByPriceListId.set(priceListId, history)
        }
      }

      const trends = ids.map(priceListId => {
        const history = historyByPriceListId.get(priceListId) || []
        const current = history[0]?.wholesale_price
        const previous = history[1]?.wholesale_price
        const hasPrices = current !== null &&
          current !== undefined &&
          current !== '' &&
          previous !== null &&
          previous !== undefined &&
        ...
        return {
          price_list_id: priceListId,
          price_trend: difference > 0 ? 'up' : 'down',
          price_change_amount: Math.abs(difference).toFixed(2)
        }
      })

      return this.createSuccessResponse('获取成功', trends)
    } catch (error) {
      log.error('批量获取价格趋势失败:', error)
      return this.createErrorResponse('获取价格趋势失败')
    }
  }
  ...
  parseProductString(productStr, inventorySet, inventoryMap) {
    ...
    // 如果没有精确匹配，尝试模糊匹配
    if (!isInInventory) {
      for (const [inventoryKey, _variants] of inventoryMap.entries()) {
        // 检查品牌是否相同
