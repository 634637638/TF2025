...
class SupplierService {
  ...
  async getSuppliers(filters = {}, options = {}) {
    try {
      ...
      const validPageSize = Math.min(Math.max(parseInt(page_size) || 100, 1), 10000)

      const result = await this.supplierRepository.getSuppliersWithPagination(
        { page: validPage, page_size: validPageSize, name, status },
        options
      )
   ...
   * 创建供应商
   */
  async createSupplier(supplierData, _user = null) {
    try {
      // 验证数据
   ...
   * 更新供应商
   */
  async updateSupplier(id, supplierData, _user = null) {
    try {
      const supplierId = parseInt(id)
      if (isNaN(supplierId) || supplierId <= 0) {
        return this.createErrorResponse('无效的供应商ID', 'INVALID_ID')
   ...
   * 删除供应商
   */
  async deleteSupplier(id, _user = null) {
    try {
      const supplierId = parseInt(id)
      if (isNaN(supplierId) || supplierId <= 0) {
        return this.createErrorResponse('无效的供应商ID', 'INVALID_ID')
   ...
   * 批量更新供应商状态
   */
  async batchUpdateStatus(ids, status, _user = null) {
    try {
      ...
      return this.createSuccessResponse('批量更新状态成功', {
        ...
        status_text: Number(status) === 1 ? '启用' : '禁用'
      })
    } catch (error) {
  ...
  async checkNameAvailability(name, exclude_id = null) {
    try {
      ...
      const isAvailable = await this.supplierRepository.checkNameAvailability(name.trim(), exclude_id)
