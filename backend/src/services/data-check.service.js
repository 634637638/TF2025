...
class DataCheckService {
   ...
   * 通用重复数据检查方法
   */
  async checkDuplicates(table_name, field_name, extraFields = []) {
    const connection = await this.getPool().getConnection()

    try {
      // 构建查询SQL
      const selectFields = ['id', field_name, 'status', 'created_at', ...extraFields].join(', ')
      const sql = `
        SELECT ${selectFields}
        FROM ${table_name}
      ...
      for (const row of rows) {
        const key = String(row[field_name] || '').trim().toLowerCase()
      ...
      for (const [key, value] of seen.entries()) {
        if (value.duplicates.length > 0) {
          duplicates.push({
            ...
            total_records: value.duplicates.length + 1
          })
        }
      }
  ...
  async checkCustomers() {
    ...
    try {
      ...
      if (duplicateRowIds.length > 0) {
        const duplicate_ids = duplicateRowIds.map(row => row.id)
        const placeholders = duplicate_ids.map(() => '?').join(',')
        ...
        for (const [id, records] of recordsGroupById.entries()) {
          if (records.length > 1) {
            ...
            duplicates.push({
              ...
              is_duplicate_rows: true
            })
          }
        }
      }

      log.debug('处理后的重复数据行数量:', duplicates.filter(d => d.is_duplicate_rows).length)
      ...
      for (const row of duplicateCustomers) {
        const ids = row.all_ids.split(',').map(id => parseInt(id))
        const primary_id = row.first_id

        // 从批量查询结果中获取记录
        const primaryRecord = allCustomerRecords.find(r => r.id === primary_id)
  ...
  async checkUsers() {
    ...
    try {
      ...
      return {
        ...
        duplicate_groups: allDuplicates.length
      }
    } finally {
      connection.release()
    }
  ...
  async checkAll() {
    ...
    return {
      ...
      summary: {
        brands: {
          total: brands.total,
          duplicate_groups: brands.duplicate_groups
        },
        models: {
          total: models.total,
          duplicate_groups: models.duplicate_groups
        },
        colors: {
          total: colors.total,
          duplicate_groups: colors.duplicate_groups
        },
        memories: {
          total: memories.total,
          duplicate_groups: memories.duplicate_groups
        },
        suppliers: {
          total: suppliers.total,
          duplicate_groups: suppliers.duplicate_groups
        },
        stores: {
          total: stores.total,
          duplicate_groups: stores.duplicate_groups
        },
        customers: {
          total: customers.total,
          duplicate_groups: customers.duplicate_groups
        },
        users: {
          total: users.total,
          duplicate_groups: users.duplicate_groups
        }
      },
      details: {
        brands,
   ...
   * 合并重复数据
   */
  async mergeDuplicates(type, primary_id, duplicate_ids, _user = null) {
    const connection = await this.getPool().getConnection()

    try {
      // ===== 安全检查 =====
      // 1. 确保 primary_id 不在 duplicate_ids 中
      ...
      return {
        ...
        message: `成功合并 ${duplicate_ids.length} 条记录到 ${primary_id}`
      }
    } catch (error) {
   ...
   */
  async batchMergeMultipleGroups(type, merge_groups, _user = null) {
    const connection = await this.getPool().getConnection()
    ...
    try {
      ...
      // 收集所有需要更新的ID和需要删除的ID
      const allUpdateIds = new Map() // Map<duplicateId, primary_id>
      const allDeleteIds = []

      for (const group of merge_groups) {
        const { primary_id, duplicate_ids } = group
        ...
        if (primaryRecords.length === 0) {
          throw new Error(`主记录 ID ${primary_id} 不存在`)
        }

        // 确保 primary_id 不在 duplicate_ids 中
        if (duplicate_ids.includes(primary_id)) {
          throw new Error('主记录ID不能在待删除列表中')
        }
        ...
        for (const dupId of duplicate_ids) {
          allUpdateIds.set(dupId, primary_id)
        }
        allDeleteIds.push(...duplicate_ids)
        total_merged += duplicate_ids.length
      }

      // 批量更新所有关联记录
      for (const relation of relations) {
        // 为每个关联表构建批量更新
        for (const [duplicateId, primary_id] of allUpdateIds.entries()) {
          await connection.execute(
            `UPDATE ${relation.table} SET ${relation.field} = ? WHERE ${relation.field} = ?`,
            [primary_id, duplicateId]
          )
        }
      }
      ...
      return {
        type,
        merged_groups: merge_groups.length,
        total_merged,
        message: `成功合并 ${merge_groups.length} 个重复组，共 ${total_merged} 条记录`
      }
    } catch (error) {
      await connection.rollback()
      throw error
   ...
   * 批量删除重复数据
   */
  async batchDeleteDuplicates(type, ids, _user = null) {
    const connection = await this.getPool().getConnection()
   ...
   * 编辑数据
   */
  async editData(type, id, data, _user = null) {
    const connection = await this.getPool().getConnection()
   ...
   * 删除单条数据
   */
  async deleteData(type, id, _user = null) {
    const connection = await this.getPool().getConnection()
    ...
    try {
      ...
      const dataList = rows.map(row => {
        ...
        return {
          ...
          duplicate_count: key ? (nameMap.get(key)?.length || 1) : 1
        }
      })
      ...
      return {
        ...
        duplicate_count: dataList.filter(item => item.is_duplicate).length,
        data: dataList,
        is_empty: false
      }
    } finally {
   ...
   */
  async cleanupData(type, _user = null, specificIds = null) {
    const connection = await this.getPool().getConnection()
    ...
    try {
      if (type === 'customers' || type === 'users') {
        ...
        const field_name = type === 'customers' ? '客户' : '员工'

        let targetIds = specificIds

        // 如果没有指定ID，则获取所有有重复的ID
        if (!targetIds) {
          const [duplicate_ids] = await connection.execute(`
            ...
            ORDER BY id
          `)
          targetIds = duplicate_ids.map(r => r.id)
          ...
          try {
            ...
            for (let j = 1; j < records.length; j++) {
              ...
              for (const field of fieldsToCompare) {
                if (String(keepRecord[field] ?? '') !== String(record[field] ?? '')) {
                  hasDifference = true
        ...
        const [remainingDuplicates] = await connection.execute(`
          SELECT COUNT(*) as count FROM (
            SELECT id, COUNT(*) as cnt FROM ${table_name} GROUP BY id HAVING cnt > 1
          ) as t
        `)

        log.debug(`剩余重复组: ${remainingDuplicates[0].count}`)

        let message = `成功清理 ${totalCleaned} 条${field_name}重复数据行，还剩 ${remainingDuplicates[0].count} 个重复组`
