...
class SharedService {
  ...
  serialize(row) {
    return {
      ...
      author: { id: row.author_id, name: row.author_name || row.author_username || '未知用户', username: row.author_username || '' }
    };
  }

  async list(userId, { page = 1, limit = 12, keyword = '', category = '', visibility = '' } = {}) {
    await this.ensure();
    const db = getDatabase();
    const pageInt = Math.max(1, Number.parseInt(page, 10) || 1);
    const limitInt = Math.min(50, Math.max(1, Number.parseInt(limit, 10) || 12));
    ...
    const [rows] = await db.query(`
      SELECT p.*, u.name author_name, u.username author_username
      FROM shared_posts p LEFT JOIN users u ON u.id=p.author_id
      ${where}
      ORDER BY p.is_pinned DESC, p.created_at DESC
      LIMIT ${limitInt} OFFSET ${(pageInt - 1) * limitInt}
    `, params);
    const total = Number(countRows[0]?.total || 0);
    return { rows: rows.map(row => this.serialize(row)), pagination: { page: pageInt, limit: limitInt, total, totalPages: Math.ceil(total / limitInt) } };
  }
  ...
  async getById(id, viewerId = null) {
    ...
    const [rows] = await getDatabase().query(`SELECT p.*, u.name author_name, u.username author_username FROM shared_posts p LEFT JOIN users u ON u.id=p.author_id WHERE p.id=?${visibilityWhere}`, params);
