...
class BrandService {
  ...
  async getBrands(filters = {}, options = {}) {
    try {
      ...
      const validPageSize = Math.min(Math.max(parseInt(page_size) || 10, 1), 100)
   ...
   * 创建品牌
   */
  async createBrand(brandData, _user = null) {
    try {
      // 验证数据
   ...
   * 更新品牌
   */
  async updateBrand(id, brandData, _user = null) {
    try {
      const brandId = parseInt(id)
      if (isNaN(brandId) || brandId <= 0) {
        return this.createErrorResponse('无效的品牌ID', 'INVALID_ID')
   ...
   * 删除品牌
   */
  async deleteBrand(id, _user = null) {
    try {
      const brandId = parseInt(id)
      if (isNaN(brandId) || brandId <= 0) {
        return this.createErrorResponse('无效的品牌ID', 'INVALID_ID')
   ...
   * 品牌搜索建议
   */
  async getBrandSuggestions(keyword, page_size = 5) {
    try {
      if (!keyword || keyword.length < 2) {
        return this.createSuccessResponse('搜索建议', [])
      }

      const suggestions = await this.brandRepository.getBrandSuggestions(keyword.trim(), page_size)
