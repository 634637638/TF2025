...
class DataImportService {
   ...
   */
  async analyzeData(filePath, _options = {}) {
    ...
    try {
      ...
      if (data.length === 0) {
        return this.toCanonicalAnalysis({
          total: 0,
          newRecords: 0,
          duplicateRecords: 0,
          duplicates: [],
        ...
        if (allSerialNumbers.length > 0 || allImeis.length > 0) {
          ...
          let query = `
            SELECT p.id, p.imei, p.serial_number, p.status, p.purchase_cost, p.sale_price,
                   p.sale_time, p.sale_operator_id,
                   ...
                   sa.id as sale_id, sa.customer_id, sa.sale_time,
            ...
            LEFT JOIN (
              SELECT phone_id, id, customer_id, sale_time,
                     ROW_NUMBER() OVER (PARTITION BY phone_id ORDER BY sale_time DESC) as rn
              FROM sales
          ...
          for (const rowToCheck of rowsToCheck) {
            const { imei, serialNumber, _customerName, _customerPhone, compositeKey, rowIndex } = rowToCheck
      ...
      return this.toCanonicalAnalysis(analysis)
    } finally {
      connection.release()
    }
  }

  // 服务内部仍使用便于算法处理的 camelCase；跨 API 边界统一输出 snake_case。
  toCanonicalAnalysis(analysis) {
    const canonicalRecord = record => {
      ...
      return {
        ...
        sale_time: record.sale_time || record.saleDate || null,
        ...
        salesman_name: record.salesmanName,
        sale_id: record.saleId,
        customer_id: record.customerId,
        has_sale: Boolean(record.hasSale)
      }
    }

    return {
      total: Number(analysis?.total || 0),
      new_records: Number(analysis?.newRecords || 0),
      duplicate_records: Number(analysis?.duplicateRecords || 0),
      summary: analysis?.summary || {},
      duplicates: Array.isArray(analysis?.duplicates)
        ? analysis.duplicates.map(duplicate => ({
          row_index: duplicate.rowIndex,
          imei: duplicate.imei,
          composite_key: duplicate.compositeKey,
          data: duplicate.data,
          existing_record: canonicalRecord(duplicate.existingRecord),
  ...
  async importData(filePath, options = {}, user = null) {
    const safeFilePath = validateSpreadsheetFile(filePath, { rootPath: IMPORT_UPLOAD_DIR })
    const connection = await this.getPool().getConnection()
    const importId = options.import_id || crypto.randomUUID()
    ...
    try {
      ...
      if (strategy === 'replace_all') {
        ...
        // 创建基础数据映射
        maps = await this.buildDataMaps(connection, data)
      }
   ...
   * 构建基础数据映射
   */
  async buildDataMaps(connection, _data) {
    const maps = {
      brands: new Map(),
      models: new Map(),
      colors: new Map(),
   ...
   * 导入单行数据
   */
  async importRow(connection, row, maps, strategy, _user = null) {
    const imei = String(row['IMEI'] || '').trim()
    const serialNumber = String(row['序列号'] || '').trim()
    const _customerName = String(row['客户姓名'] || '').trim()
    const _customerPhone = String(row['手机号'] || '').trim()
    ...
    if (imei || serialNumber) {
      ...
      `
      const params = [serialNumber || '', imei || '']
    ...
    if (duplicateRecord) {
      ...
      const _salePrice = parseFloat(row['销售价']) || null

      // 判断Excel中的销售状态
      const hasSaleDate = !!saleDate
      const hasCustomerInfo = !!customerName
      const hasPhone = !!phone
      const _hasSaleOperator = !!saleOperator
          ...
          if (status === 'sold' && saleDate) {
            if (customerId) {
              ...
              if (existingSales.length > 0) {
                // 更新现有销售记录
                await connection.execute(
                  'UPDATE sales SET customer_id = ?, sale_time = ?, operator_id = ? WHERE phone_id = ?',
                ...
                // 创建新的销售记录
                await connection.execute(
                  `INSERT INTO sales (phone_id, customer_id, sale_time, operator_id)
                   VALUES (?, ?, ?, ?)`,
    ...
    const [result] = await connection.execute(
      `INSERT INTO phones (
        ...
        supplier_id, store_id, status, sale_time, inventory_time,
    ...
    if (status === 'sold') {
      if (!saleDate) {
        ...
        const [saleResult] = await connection.execute(
          `INSERT INTO sales (phone_id, customer_id, sale_time, operator_id, status)
           VALUES (?, ?, ?, ?, 'completed')`,
