...
class ShopService {
   ...
   * @param {Array<number>} image_ids - 按新顺序排列的图片ID数组
   */
  async reorderPhoneImages(phoneId, image_ids) {
    ...
    try {
      ...
      const [images] = await connection.query(
        'SELECT id FROM H5_images WHERE phone_id = ? FOR UPDATE',
        [phoneId]
      )
      const valid_image_ids = new Set(images.map(image => image.id))

      for (const image_id of image_ids) {
        if (!valid_image_ids.has(image_id)) {
          throw new Error(`图片ID ${image_id} 不属于该商品`)
        }
      }
      ...
      for (let i = 0; i < image_ids.length; i++) {
        await connection.query(
          ...
          [i, image_ids[i], phoneId]
        )
      }

      await connection.commit()
      log.debug(`[reorderPhoneImages] 商品 ${phoneId} 图片排序已更新:`, image_ids)
    } catch (error) {
      await connection.rollback()
      throw error
    } finally {
   ...
   * 获取订单列表
   */
  async getOrders({ page, page_size, status, start_date, end_date, search }) {
    ...
    // 查询数据
    const query = `
      SELECT ${selectFields(H5_ORDER_FIELDS, 'o')}, COUNT(oi.id) as item_count
      FROM H5_orders o
   ...
   * 更新订单状态
   */
  async updateOrderStatus(orderId, status, _userId) {
    const query = `
      UPDATE H5_orders
      SET status = ?, updated_at = NOW()
      WHERE id = ?
    `
    await db.getDatabase().query(query, [status, orderId])
  }
