...
class EmployeeService {
   ...
   * 创建员工
   */
  async createEmployee(employeeData, _user = null) {
    try {
      // 验证数据
      ...
      const { username, password, _name } = employeeData
   ...
   * 更新员工
   */
  async updateEmployee(id, employeeData, _user = null) {
    try {
      const employeeId = parseInt(id)
      if (isNaN(employeeId) || employeeId <= 0) {
        return this.createErrorResponse('无效的员工ID', 'INVALID_ID')
  ...
  async batchUpdateStatus(ids, status, user = null) {
    try {
      ...
      // 防止禁用自己
      if (user && Number(status) === 0 && validIds.includes(Number(user.id))) {
