...
class QueryService {
  ...
  async getComprehensiveQuery(filters, userStoreId = null, userStoreIds = []) {
    try {
      ...
      const cleanFilters = {
        ...
        page_size: parseInt(processArrayParam(filters.page_size)) || 20,
        ...
        sort_field: processArrayParam(filters.sort_field) ? processArrayParam(filters.sort_field).trim() : 'sale_time',
        ...
        if (userStoreIds && userStoreIds.length > 0) {
          ...
          return {
            ...
            pagination: {
              ...
              page_size: cleanFilters.page_size,
              total: 0,
              total_pages: 0,
              has_next: false,
              has_prev: false
      ...
      // 验证排序字段 - 更新为新的字段名
      const allowedSortFields = ['business_time', 'inventory_time', 'sale_time', 'brand', 'model', 'sale_price', 'purchase_cost']
      if (!allowedSortFields.includes(cleanFilters.sort_field)) {
        cleanFilters.sort_field = 'sale_time'
      }
      ...
      const formattedData = result.data.map(item => {
        ...
        return ({
          ...
          price_info: {
            wholesale_price: item.wholesale_price === null ? null : Number(item.wholesale_price),
            retail_price: item.retail_price === null ? null : Number(item.retail_price),
            purchase_cost: item.purchase_cost === null ? null : Number(item.purchase_cost),
            sale_price: item.sale_price === null ? null : Number(item.sale_price),
            sales_record_price: item.sales_record_price === null ? null : Number(item.sales_record_price)
          },
          time_info: {
            inventory_time: item.inventory_time,
            // 销售时间优先使用销售记录，其次使用手机记录，最后使用销售创建时间。
            // 确保不返回空字符串，如果为空则返回 null
            sale_time: item.sales_sale_time || item.sale_time || item.phones_sale_time || item.sale_created_at || null
          },
          customer_info: {
  ...
  async getReturnGoodsRecords(filters = {}) {
    try {
      const cleanFilters = {
        ...
        page_size: parseInt(filters.page_size, 10) || 20,
  ...
  buildExcelRows(items = []) {
    return items.map(item => ({
      手机ID: item?.basic_info?.phone_id || '',
      IMEI: item?.basic_info?.imei || '',
      序列号: item?.basic_info?.serial_number || '',
      品牌: item?.basic_info?.brand || '',
      型号: item?.basic_info?.model || '',
      颜色: item?.basic_info?.color || '',
      内存: item?.basic_info?.memory || '',
      新旧: item?.basic_info?.is_new === 1 ? '全新' : item?.basic_info?.is_new === 0 ? '二手' : '',
      机况: item?.basic_info?.quality_grade || '',
      状态: item?.basic_info?.status || '',
      备注: item?.basic_info?.remarks || '',
      供应商: item?.supplier_info?.supplier_name || '',
      供应商联系人: item?.supplier_info?.supplier_contact || '',
      供应商电话: item?.supplier_info?.supplier_phone || '',
      店铺: item?.store_info?.store_name || '',
      店铺地址: item?.store_info?.store_address || '',
      入库成本价: item?.price_info?.purchase_cost || 0,
      批发价: item?.price_info?.wholesale_price || 0,
      零售价: item?.price_info?.retail_price || 0,
      实际销售价: item?.price_info?.sale_price || 0,
      销售记录价: item?.price_info?.sales_record_price || 0,
      入库时间: item?.time_info?.inventory_time || '',
      销售时间: item?.time_info?.sale_time || '',
      客户姓名: item?.customer_info?.customer_name || '',
      客户电话: item?.customer_info?.customer_phone || '',
      AppleID: item?.customer_info?.apple_id || '',
      销售员: item?.operator_info?.sale_operator_name || '',
      入库员: item?.operator_info?.inventory_operator_name || '',
      销售类型: item?.sale_info?.sale_type || '',
      支付方式: item?.sale_info?.payment_method || '',
      支付渠道: item?.sale_info?.payment_channel || '',
      发票号: item?.sale_info?.invoice_number || '',
      销售备注: item?.sale_info?.sale_remarks || ''
    }))
  }
