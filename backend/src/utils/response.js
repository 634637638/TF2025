...
class ApiResponse {
  ...
  static error(res, arg2, arg3, arg4) {
    ...
    if (typeof arg3 === 'number') {
      ...
      if (code) {
        return res.status(statusCode).json({
          success: false,
          message: Number(statusCode) >= 500 ? '服务器内部错误，请稍后重试' : message,
          code
        })
      }
    }
    ...
    const response = {
      success: false,
      message: isServerError ? '服务器内部错误，请稍后重试' : message,
      timestamp: new Date().toISOString()
    }

    if (!isServerError && error !== null && error !== undefined) {
  ...
  static created(res, arg2, arg3, arg4) {
    ...
    if (typeof arg2 === 'string') {
      ...
      // 原api-response.js方式: created(res, data, message, statusCode)
      data = arg2 ?? null
      message = arg3 || '创建成功'
      statusCode = arg4 || 201
    }
   ...
   * 服务器错误响应
   */
  static serverError(res, _message = '服务器内部错误', _error = null) {
    const response = {
      success: false,
