...
class SaleRepository extends BaseRepository {
    ...
    try {
      const query = `
        SELECT
          ...
          ph.inventory_time as inventory_time,
          ph.sale_time as sale_time
        FROM phones ph
  ...
  async getAvailablePhones(filters = {}) {
    try {
      ...
      const limitVal = Number(page_size)
      ...
      const fullQuery = `
        ...
        LEFT JOIN users inv_op ON ph.inventory_operator_id = inv_op.id
        WHERE ${whereClause}
        ORDER BY ph.inventory_time DESC
      ...
      const processedRecords = records.map(normalizePhoneRecord)
    ...
    try {
      ...
      const query = `
        SELECT
          ...
          COUNT(DISTINCT ph.brand_id) as brand_count,
          COUNT(DISTINCT ph.model_id) as model_count,
          SUM(CASE WHEN ph.is_new = 1 THEN 1 ELSE 0 END) as new_count,
          SUM(CASE WHEN ph.is_new = 0 THEN 1 ELSE 0 END) as used_count,
          AVG(ph.purchase_cost) as avg_cost
        FROM phones ph
        WHERE ${whereClause}
      `
  ...
  async checkPhoneAvailability(phoneId) {
    try {
      const query = `
        SELECT ${PHONE_STOCK_BASE_FIELDS}, status
        FROM phones
        WHERE id = ? AND status = 'in_stock'
      `
  ...
  async createSale(saleData) {
    try {
      const query = `
        INSERT INTO sales (
          phone_id, customer_id, sale_type, sale_price, purchase_cost,
          operator_id, store_id, payment_method, invoice_number, sale_time
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `
  ...
  async getSaleStats(filters = {}) {
    try {
      ...
      if (start_date) {
        whereConditions.push('DATE(sr.sale_time) >= ?')
      ...
      const query = `
        SELECT
          ...
          SUM(sr.purchase_cost) as total_cost,
  ...
  async getSaleRecords(filters = {}) {
    try {
      ...
      const query = `
        ...
        WHERE ${whereClause}
        ORDER BY sr.sale_time DESC, sr.created_at DESC
        LIMIT ? OFFSET ?
      `

      const records = await this.executeQuery(query, [...queryParams, parseInt(page_size), parseInt(offset)])
    ...
    try {
      ...
      const query = `
        SELECT
          ...
          COUNT(CASE WHEN ph.status = 'in_stock' THEN 1 END) as in_stock_count,
          COUNT(CASE WHEN ph.status = 'sold' THEN 1 END) as sold_count,
          COUNT(CASE WHEN ph.status = 'reserved' THEN 1 END) as reserved_count,
          COUNT(CASE WHEN ph.status = 'damaged' THEN 1 END) as damaged_count,
          COUNT(CASE WHEN ph.is_new = 1 THEN 1 END) as new_count,
          COUNT(CASE WHEN ph.is_new = 0 THEN 1 END) as used_count,
          SUM(CASE WHEN ph.status = 'in_stock' THEN ph.purchase_cost ELSE 0 END) as total_stock_value,
          AVG(CASE WHEN ph.status = 'in_stock' THEN ph.purchase_cost ELSE NULL END) as avg_stock_cost,
          COUNT(DISTINCT ph.brand_id) as brand_count,
          COUNT(DISTINCT ph.model_id) as model_count,
  ...
  async batchUpdateSaleInfo(updates) {
    ...
    try {
      ...
      const saleIdCases = updates.map((_u, _i) => 'WHEN ? THEN ?').join(' ')
      const saleIdParams = updates.flatMap(u => [u.phoneId, u.saleId])

      // 构建 sale_operator_id 的 CASE 语句
      const saleOperatorIdCases = updates.map((_u, _i) => 'WHEN ? THEN ?').join(' ')
