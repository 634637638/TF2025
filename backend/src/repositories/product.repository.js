...
class ProductRepository extends BaseRepository {
  ...
  async getModels(filters = {}) {
    try {
      ...
      let query = `
        ...
        LEFT JOIN phones ph ON m.id = ph.model_id AND ph.status = 'in_stock'
      `
    ...
    try {
      const query = `
        SELECT
          ...
          COUNT(DISTINCT ph.id) as phone_count
        FROM colors c
        LEFT JOIN phones ph ON c.id = ph.color_id AND ph.status = 'in_stock'
        GROUP BY c.id
        ORDER BY c.name
      `
    ...
    try {
      const query = `
        SELECT
          ...
          COUNT(DISTINCT ph.id) as phone_count
        FROM memories m
        LEFT JOIN phones ph ON m.id = ph.memory_id AND ph.status = 'in_stock'
        GROUP BY m.id
  ...
  async getInStockPhones(filters = {}) {
    try {
      ...
      let query = `
        ...
        LEFT JOIN colors c ON ph.color_id = c.id
        LEFT JOIN memories mem ON ph.memory_id = mem.id
        LEFT JOIN suppliers s ON ph.supplier_id = s.id
        LEFT JOIN stores st ON ph.store_id = st.id
        WHERE ph.status = 'in_stock'
      `
  ...
  async updatePhone(id, phoneData) {
    ...
    try {
      ...
      // Keep historical callers at this repository boundary; all SQL uses canonical columns.
      const canonicalPhoneData = { ...phoneData }
      const legacyToCanonical = {
        purchase_price: 'purchase_cost',
        purchase_unit_price: 'purchase_cost',
        cost_price: 'purchase_cost',
        Inventorytime: 'inventory_time',
        stock_in_date: 'inventory_time',
        salestime: 'sale_time',
        sale_date: 'sale_time'
      }
      for (const [legacyField, canonicalField] of Object.entries(legacyToCanonical)) {
        if (canonicalPhoneData[canonicalField] === undefined && canonicalPhoneData[legacyField] !== undefined) {
          canonicalPhoneData[canonicalField] = canonicalPhoneData[legacyField]
        }
        delete canonicalPhoneData[legacyField]
      }

      const fields = []
      const params = []

      Object.keys(canonicalPhoneData).forEach(key => {
        if (canonicalPhoneData[key] !== undefined) {
          fields.push(`${key} = ?`)
          params.push(canonicalPhoneData[key])
        }
      })
      ...
      // 🔥 处理客户信息更新（在事务中）
      if (canonicalPhoneData.customer_phone) {
        log.debug('处理客户信息更新，手机号:', canonicalPhoneData.customer_phone)
        ...
        if (salesRecords.length > 0) {
          ...
          const [matchedCustomers] = await connection.query(
            ...
            [canonicalPhoneData.customer_phone]
          )
          ...
          if (matchedCustomers.length > 0) {
            ...
            if (matchedCustomer.id === currentCustomerId) {
              ...
              if (canonicalPhoneData.customer_name && canonicalPhoneData.customer_name !== matchedCustomer.name) {
                await connection.query(
                  'UPDATE customers SET name = ?, updated_at = NOW() WHERE id = ?',
                  [canonicalPhoneData.customer_name, matchedCustomer.id]
                )
                log.debug(`场景二：更新客户姓名，ID=${matchedCustomer.id}, 新姓名=${canonicalPhoneData.customer_name}`)
              }
            } else {
            ...
            const [newCustomer] = await connection.query(
              ...
              [canonicalPhoneData.customer_name || null, canonicalPhoneData.customer_phone]
            )
      ...
      log.debug('接收到的数据:', {
        sale_price: canonicalPhoneData.sale_price,
        purchase_cost: canonicalPhoneData.purchase_cost,
        store_id: canonicalPhoneData.store_id,
        sale_time: canonicalPhoneData.sale_time,
        payment_method: canonicalPhoneData.payment_method
      })
      ...
      if (phoneInfo && phoneInfo.length > 0 && phoneInfo[0].sale_id) {
        ...
        // 同步销售时间
        if (canonicalPhoneData.sale_time) {
          const saleDate = canonicalPhoneData.sale_time
          salesFields.push('sale_time = ?')
          salesParams.push(saleDate)
          log.debug(`同步 sales 表 sale_time: ${saleDate}`)
        }

        // 同步销售价格
        if (canonicalPhoneData.sale_price !== undefined) {
          salesFields.push('sale_price = ?')
          salesParams.push(parseFloat(canonicalPhoneData.sale_price))
          log.debug(`同步 sales 表 sale_price: ${canonicalPhoneData.sale_price}`)
        }

        // 同步采购成本
        if (canonicalPhoneData.purchase_cost !== undefined) {
          const cost = parseFloat(canonicalPhoneData.purchase_cost || 0)
          salesFields.push('purchase_cost = ?')
          salesParams.push(cost)
          log.debug(`同步 sales 表 purchase_cost: ${cost}`)
        }

        // 同步销售门店
        if (canonicalPhoneData.store_id !== undefined) {
          salesFields.push('store_id = ?')
          salesParams.push(parseInt(canonicalPhoneData.store_id))
          log.debug(`同步 sales 表 store_id: ${canonicalPhoneData.store_id}`)
        }

        // 同步销售员
        if (canonicalPhoneData.sale_operator_id !== undefined || canonicalPhoneData.operator_id !== undefined) {
          const operatorId = parseInt(canonicalPhoneData.sale_operator_id || canonicalPhoneData.operator_id || 0)
          if (operatorId > 0) {
        ...
        // 同步支付方式
        if (canonicalPhoneData.payment_method !== undefined) {
          salesFields.push('payment_method = ?')
          salesParams.push(canonicalPhoneData.payment_method)
          log.debug(`同步 sales 表 payment_method: ${canonicalPhoneData.payment_method}`)
        }

        // 同步备注
        if (canonicalPhoneData.remarks !== undefined) {
          salesFields.push('remarks = ?')
          salesParams.push(canonicalPhoneData.remarks)
          log.debug(`同步 sales 表 remarks: ${canonicalPhoneData.remarks}`)
        }
