...
const sanitizeSystemSettings = (settings = {}) => ({
  backgroundType: ['default', 'image', 'video'].includes(settings.backgroundType)
    ? settings.backgroundType
    : DEFAULT_SYSTEM_SETTINGS.backgroundType,
...
async function loadSystemSettings() {
  try {
    ...
    return sanitizeSystemSettings(JSON.parse(data))
  } catch (error) {
...
router.post('/', unifiedAuth, requirePermission('system:edit'), async (req, res) => {
  try {
    ...
    const sanitizedSettings = sanitizeSystemSettings(settings)

    // 保存设置
    const saved = await saveSystemSettings(sanitizedSettings)
...
router.post('/upload/screen-lock',
  ...
  async (req, res) => {
    try {
      ...
      const hasValidSignature = await validateUploadedFileSignature(req.file, ['image', 'video'])
