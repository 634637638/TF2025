...
// 获取手机入库中使用的颜色和内存选项
router.get('/phone-options', requireAnyPermission(['accessories:view', 'accessories:create']), async (req, res) => {
  try {
    if (!isConnected()) {
    ...
    const _colorOptions = [...colors.map(row => String(row.name)), ...actualColors.map(row => String(row.color))]
