...
router.get('/', unifiedAuth, requirePermission('inventory:view'), async (req, res) => {
  try {
    ...
    const pageNumber = Math.max(parseInt(page, 10) || PAGINATION.DEFAULT_PAGE, 1)
    const pageSize = Math.min(Math.max(parseInt(page_size, 10) || PAGINATION.DEFAULT_LIMIT, 1), PAGINATION.MAX_LIMIT)
    ...
    try {
      ...
      if (date) {
        if (date.includes(',')) {
          ...
          whereConditions.push('p.inventory_time BETWEEN ? AND ?')
      ...
      if (search) {
        ...
        // 其他情况进行全字段模糊搜索
          ...
          whereConditions.push(`(
            ...
            DATE(p.inventory_time) LIKE ? OR
            DATE(p.sale_time) LIKE ?
          )`)
      ...
      const dataQuery = `
        SELECT p.id, p.imei, p.serial_number, p.brand_id, p.model_id, p.color_id, p.memory_id, p.purchase_cost, p.sale_price, p.is_new, p.status, p.quality_grade, p.remarks, p.purchase_number, p.inventory_operator_id, p.sale_operator_id, p.supplier_id, p.store_id, p.inventory_time, p.sale_time,
      ...
      const [phones] = await connection.execute(dataQuery, [...queryParams, pageSize, offset])

      // 处理返回数据格式
      const processedPhones = phones.map(phone => ({
        ...phone,
        purchase_cost: phone.purchase_cost ?? null,
        sale_price: phone.sale_price ?? null,
        inventory_time: phone.inventory_time || null,
        sale_time: phone.sale_time || null,
        condition: Number(phone.is_new) === 1 ? 'new' : 'used'
      }))
      ...
      ApiResponse.success(res, {
        ...
        pagination: {
          ...
          has_next: pageNumber < Math.ceil(total / pageSize),
    ...
    try {
      let query = `
        SELECT
          ...
          co.name as color,
          mem.size as memory,
          p.inventory_time as inventory_time,  -- 入库时间
          latest_sale.sale_time as sale_time,        -- 出库时间（销售时间）
          c.name as customer_name,
          c.phone as customer_phone,
          st.name as store_name
        FROM phones p
...
router.get('/longest-inventory', requireInventoryQueryToken, async (req, res) => {
  ...
  let connection
  try {
    const { brand_id, model_id, color_id, memory_id, store_id } = req.query
    const filterIds = { brand_id, model_id, color_id, memory_id, store_id }
    for (const [field, value] of Object.entries(filterIds)) {
      if (value !== undefined && (!Number.isInteger(Number(value)) || Number(value) <= 0)) {
        return ApiResponse.badRequest(res, `${field} 必须是正整数`)
      }
    }
    ...
    if (brand_id) {
      whereConditions.push('p.brand_id = ?')
      queryParams.push(Number(brand_id))
    }
    if (model_id) {
      whereConditions.push('p.model_id = ?')
      queryParams.push(Number(model_id))
    }
    if (color_id) {
      whereConditions.push('p.color_id = ?')
      queryParams.push(Number(color_id))
    }
    if (memory_id) {
      whereConditions.push('p.memory_id = ?')
      queryParams.push(Number(memory_id))
    }
    if (store_id) {
      whereConditions.push('p.store_id = ?')
...
router.put('/:id', unifiedAuth, requireAnyPermission(['phones:edit', 'sales-editphoneview:edit', 'query:edit', 'inventory:edit']), async (req, res) => {
  ...
  try {
    ...
    const [existingSales] = await connection.execute(
      'SELECT id, customer_id, operator_id, sale_time, store_id, remarks, invoice_number, sale_price, purchase_cost, sale_type FROM sales WHERE phone_id = ? ORDER BY created_at DESC LIMIT 1',
      [id]
    )
    ...
    if ((finalCustomerId || customer_name || customer_phone) && existingSales.length === 0) {
      ...
      const [insertResult] = await connection.execute(
        `INSERT INTO sales (
          ...
          sale_price, purchase_cost, sale_time, invoice_number, remarks, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [
          id,
    ...
    const [updatedPhones] = await connection.execute(
      `SELECT p.id, p.imei, p.serial_number, p.brand_id, p.model_id, p.color_id, p.memory_id, p.sale_price, p.purchase_cost, p.is_new, p.status, p.quality_grade, p.remarks, p.purchase_number, p.inventory_operator_id, p.sale_operator_id, p.supplier_id, p.store_id, p.inventory_time, p.sale_time,
              s.name as supplier_name,
              st.name as store_name,
              su.name as sale_operator_name,
              inv_u.name as inventory_operator_name,
    ...
    if (effectiveStatus === 'in_stock') {
      ...
      try {
        ...
        const [matchablePreorders] = await connection.execute(
          `SELECT p.id, p.preorder_number, p.customer_id, c.name AS customer_name, c.phone AS customer_phone,
                  p.brand_id, p.model_id, p.color_id, p.memory_id, p.total_price, p.deposit_amount
           FROM preorders p
...
router.post('/:id/upload-video',
  ...
  async (req, res) => {
    try {
      ...
      if (!(await validateUploadedFileSignature(req.file, ['video']))) {
        await removeUploadedFiles([req.file])
        return ApiResponse.error(res, '文件内容与视频格式不匹配', 400)
      }
