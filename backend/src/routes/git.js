...
const createGitError = (message, code, details = {}) => Object.assign(
  new Error(message),
...
const execGitArgs = (args) => {
  return new Promise((resolve, reject) => {
    execFile('git', args, getGitExecOptions(), (error, stdout, stderr) => {
      if (error) {
        reject(createGitError(error.message, error.code, {
          stderr: stderr.trim(),
          killed: error.killed
        }))
      } else {
...
const normalizeBranchName = (branch, fieldName = '分支名称') => {
  ...
  if (!GIT_BRANCH_NAME_PATTERN.test(normalizedBranch)) {
    throw createGitError(`${fieldName}不合法`, 'INVALID_BRANCH')
  }
...
const normalizeCommitHash = (commit) => {
  ...
  if (!GIT_COMMIT_HASH_PATTERN.test(normalizedCommit)) {
    throw createGitError('提交哈希不合法', 'INVALID_COMMIT')
  }
...
const createBranchBackupSnapshot = async ({
  ...
  try {
    ...
    throw createGitError(`备份分支已存在: ${backupBranch}`, 'BRANCH_EXISTS')
  } catch (error) {
