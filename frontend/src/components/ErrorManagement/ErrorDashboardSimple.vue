<template>
  <div class="error-dashboard admin-page">
    ...
    <!-- 最常见错误 -->
    <el-card
      v-if="statistics.mostCommonErrors.length > 0"
      class="common-errors-card"
    >
      <template #header>
        <span>最常见错误</span>
      </template>
      <el-table
        :data="statistics.mostCommonErrors.slice(0, 5)"
    ...
    <el-card class="error-list-card">
      ...
      >
        ...
        <el-table-column
          label="操作"
          :width="$getActionColumnWidth(['详情', '标记解决', '删除'])"
          align="center"
        ...
        >
          <template #default="{ row }">
            <div class="action-buttons table-actions">
              <el-button
                size="small"
                @click.stop="showErrorDetail(row)"
              >
                详情
              </el-button>
              <el-button
                ...
                size="small"
                type="success"
                @click.stop="resolveError(row.id)"
              >
                标记解决
              </el-button>
              <el-button
    ...
    >
      ...
      >
        ...
        <div
          v-if="selectedError.tags?.length"
          class="error-section"
        >
          <h4>标签</h4>
          <el-space wrap>
            <el-tag
              v-for="tag in selectedError.tags"
              :key="tag"
              size="small"
            >
              {{ tag }}
...
.error-dashboard {
  ...
  :deep(.error-row-critical) {
    background-color: var(--tf-color-danger-surface-element);
  }

  :deep(.error-row-high) {
    background-color: var(--tf-color-warning-surface-element);
  }
