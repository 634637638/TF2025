  ...
  <div class="accessories-view admin-page admin-unified-base-data-page">
    <PageHeader icon="fas fa-box" title="配件管理">
      <template #actions>
        ...
        <el-button type="info" plain :disabled="refreshing" @click="handleRefresh">
    ...
    <div class="accessories-table-container admin-page-content">
      <div class="table-container table-section admin-panel admin-table-panel">
        ...
        <div class="table-responsive">
          ...
          >
            <template #empty>
              <TableLoadingRow v-if="loading" mode="block" text="加载配件数据中..." />
              <div v-else-if="error" class="table-state table-error-state">
                <i class="fas fa-exclamation-triangle"></i>
                <p>{{ error }}</p>
                <el-button type="primary" size="small" @click="loadAccessories()">重试</el-button>
              </div>
              <div v-else class="empty-state">
                <i class="fas fa-box-open"></i>
                <p>暂无配件数据</p>
                <el-button v-if="canCreate" type="primary" size="small" @click="openStockInModal">
                  配件入库
                </el-button>
              </div>
            </template>

            <el-table-column v-if="!isMobile" label="序号" width="64" align="center">
            ...
            <el-table-column v-if="!isMobile" prop="brand_name" label="品牌" min-width="104" align="center">
              <template #default="{ row }">{{ row.brand_name || '-' }}</template>
            </el-table-column>
            <el-table-column v-if="!isMobile" prop="model_name" label="型号" min-width="116" align="center">
              <template #default="{ row }">{{ row.model_name || '-' }}</template>
            </el-table-column>
            <el-table-column v-if="!isMobile" label="进价" min-width="96" align="center">
              <template #default="{ row }"><span class="price">¥{{ formatPrice(row.purchase_price) }}</span></template>
            </el-table-column>
            <el-table-column v-if="!isMobile" label="售价" min-width="96" align="center">
              <template #default="{ row }"><span class="price">¥{{ formatPrice(row.selling_price) }}</span></template>
            </el-table-column>
            <el-table-column v-if="!isMobile" label="毛利" min-width="96" align="center">
              <template #default="{ row }">
                <span :class="['profit-value', getProfit(row) >= 0 ? 'profit-positive' : 'profit-negative']">
                  ¥{{ formatPrice(getProfit(row)) }}
                </span>
              </template>
            </el-table-column>
            <el-table-column v-if="!isMobile" label="总库存" min-width="102" align="center">
              <template #default="{ row }">
                <span :class="['status-badge', getStockStatusBadgeClass(row)]">
                  {{ getTotalStock(row) }} {{ row.unit || '件' }}
                </span>
              </template>
            </el-table-column>
            <el-table-column v-if="!isMobile" label="已售" min-width="74" align="center">
              <template #default="{ row }">{{ row.total_out || 0 }}</template>
            </el-table-column>
            <el-table-column label="剩余" :min-width="isMobile ? 72 : 78" align="center">
              <template #default="{ row }">
                <span :class="getRemaining(row) < 10 ? 'stock-low' : 'stock-normal'">
                  {{ getRemaining(row) }}
                </span>
              </template>
            </el-table-column>
            <el-table-column v-if="!isMobile" prop="unit" label="单位" min-width="70" align="center">
              <template #default="{ row }">{{ row.unit || '件' }}</template>
            </el-table-column>
            <el-table-column label="状态" :min-width="isMobile ? 72 : 80" align="center">
              <template #default="{ row }">
            ...
            >
              <template #default="{ row }">
                <div class="mobile-row-actions">
                  <el-button type="primary" size="small" @click.stop="viewAccessoryDetails(row)">
                    <i class="fas fa-eye"></i><span>详情</span>
                  </el-button>
                  <el-button v-if="canEdit" type="success" size="small" @click.stop="editAccessory(row)">
                    <i class="fas fa-edit"></i><span>编辑</span>
                  </el-button>
                  <el-button v-if="canDelete" type="danger" size="small" @click.stop="deleteAccessory(row)">
...
const getProfit = (accessory: AccessoryItem) => (
  Number(accessory.selling_price || 0) - Number(accessory.purchase_price || 0)
)

const getRemaining = (accessory: AccessoryItem) => {
  if (accessory.total_in !== undefined && accessory.total_in !== null) {
    return Number(accessory.total_in || 0) - Number(accessory.total_out || 0)
  }
  return getTotalStock(accessory)
}
...
const deleteAccessory = async (accessory: AccessoryItem) => {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除 ${accessory.name || '该配件'} 吗？此操作不可撤销。`, '删除确认', {
