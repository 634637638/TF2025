...
- TAB 数量超出可视区域时，Element Plus 自动生成的左右切换按钮必须使用全局固定尺寸、独立边框、浅色背景和悬停反馈，不得显示为裸箭头或覆盖首尾 TAB。
- 导航容器使用全局浅灰蓝背景，与白色内容卡片形成轻度层次；每个未选中项必须使用浅色背景、独立细边框和轻阴影，避免多个 TAB 视觉粘连。
- 选中项使用全局主色实底、同色边框和强调阴影；悬停项使用白色背景并加深边框，三种状态必须能被快速区分。
