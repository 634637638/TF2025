...
5. [前端接入规范](#前端接入规范)
6. [后端接入规范](#后端接入规范)
7. [新增功能权限接入流程](#新增功能权限接入流程)
8. [字段级权限控制](#字段级权限控制)
9. [模块扫描与注册规则](#模块扫描与注册规则)
10. [禁用做法](#禁用做法)
11. [落地检查清单](#落地检查清单)
12. [更新日志](#更新日志)
...
- `code` 只允许在创建角色时填写；留空时系统自动生成 `role_<角色ID>`。
- 角色创建后 `code` 必须保持只读，前端不得提供可编辑入口，后端更新接口必须拒绝编码变更。
- 角色名称和描述可以修改；用户绑定及角色权限继续通过 `role_id` 关联。
...
当页面后续新增按钮、批量操作或接口能力时，必须同步更新模块动作配置。权限管理页面只展示模块声明过的动作，不会为所有模块默认显示 `import/export/approve/manage` 等通用开关。
...
其中 `MODULE_PERMISSION_TYPES` 是权限管理页面展示动作的来源。新增功能时，先确认该模块是否已经声明了对应动作。

示例：Git 仓库新增“导出日志”功能前，原配置为：
...
页面新增操作入口时，需要使用统一权限判断控制显示或点击：

```ts
const { canExport, requirePermission } = usePagePermissions('git-management')
...
按钮模板中也可以使用已有权限指令，但权限码必须和模块动作配置一致。

### 后端接口校验

新增接口必须同步添加后端权限校验，前端按钮隐藏不能替代接口鉴权：

```js
router.get(
  '/git/export-logs',
  unifiedAuth,
  requirePermission('git-management:export'),
  exportGitLogs
)
```

如果后端路由使用完整模块 key，也应保证能映射到同一权限含义，例如 `system_gitmanagement:export`。

### 权限页面显示规则

- 页面显示文案可以是中文，例如“查看”“导出”。
- 数据库存储和代码判断必须使用英文动作，例如 `view`、`export`。
- `menu_view` 只代表菜单显示，不代表页面可访问。
- 未在 `MODULE_PERMISSION_TYPES` 中声明的动作，不应出现在该模块的权限管理页。

### 新功能检查清单

新增页面功能时至少检查：

1. `MODULE_PERMISSION_TYPES` 是否声明了新动作。
2. 权限管理页是否出现了对应开关。
3. 前端按钮、操作列或批量操作是否接入权限判断。
4. 后端接口是否接入 `requirePermission(...)`。
5. 当前角色是否已在权限管理页勾选新动作。
6. 修改权限后是否重新登录或刷新权限缓存。
...
1. 页面是否已注册到 `modules`。
2. `MODULE_PERMISSION_TYPES` 是否只声明真实存在的页面动作。
3. 页面使用的权限字符串是否为 `module_key:action`。
4. 路由守卫是否基于统一权限结果判断。
5. 左侧菜单是否基于菜单可见性结果渲染。
6. 按钮、操作列、批量操作是否接入统一权限判断。
7. 敏感字段是否接入字段级权限控制。
8. 后端新增接口是否接入 `requirePermission(...)`。
9. 是否仍存在按角色名硬编码放行逻辑。
10. 是否仍存在旧格式权限字符串或中文权限代码。
...
- 明确角色编码是创建后不可修改的稳定标识。
- 要求角色编辑页面只读展示编码，并由后端拒绝绕过页面的编码变更请求。

### 2026-05-31 - v1.1.3

- 补充新增功能时的权限动作接入流程。
- 明确 `MODULE_PERMISSION_TYPES` 是权限管理页动作展示来源。
- 明确页面显示中文、数据库和代码使用英文权限动作。
- 增加新增功能权限检查清单。
