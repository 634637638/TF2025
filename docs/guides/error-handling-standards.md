...
- 缺少令牌、令牌无效、令牌过期或已撤销使用 `401` 和对应的 `TOKEN_*` 错误码。
- 数据库超时、拒绝连接、连接重置或连接池不可用统一使用 `503/DB_CONNECTION_ERROR`。
- 数据库暂时不可用不能包装成 `401`，前端收到 `503` 时必须保留当前登录态，待服务恢复后允许用户重试。
- 所有受保护接口统一由 `unified-auth.js` 执行该分类，不允许业务路由自行把数据库异常改写为认证失败。
