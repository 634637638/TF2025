...
## [2026-08-30] - 上传文件业务归档与生命周期统一

### 改进优化

- 国补照片按客户或代办人及序列号归档，二手机国补照片按序列号和入库日期归档。
- 二手机图片、视频统一使用 `序列号-YYYYMMDD` 目录，并在序列号或入库日期变化时同步目录和 URL。
- H5 商城按 `h5_newimages`、`h5_banners`、`h5_config` 分模块归档，商品模板使用 `品牌-型号-颜色` 子目录。
- 上传入口统一真实文件签名校验、失败清理、删除清理和空目录清理。

### 问题修复

- 修复综合查询手机图片实际写入 `shop`、URL 却指向 `phones` 的目录错位。
- 修复删除商品模板未清理物理媒体、模板媒体写库失败遗留文件的问题。
- 修复修改国补姓名、代办人、手机序列号或入库日期后目录名与数据库 URL 不同步的问题。
- 修复已售商品、综合查询和 H5 上架编辑中的视频被图片查看器当作下载文件的问题，统一支持站内视频预览。

### 文档更新

- 新增 `docs/backend/upload-storage-standard.md`。
- 更新后端索引、项目总索引、部署索引和云端部署指南。
- 补充媒体预览组件和视频播放约定。
